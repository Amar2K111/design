...
export default function ProofWidget() {
  return (
    <section id="preuve" className="bg-surface px-6 pb-24 pt-4">
      <div className="mx-auto max-w-content">
        <h2 className="font-display text-[2rem] font-bold leading-tight text-foreground sm:text-[2.2rem]">
        ...
        <div className="surface-brand relative mt-6 p-3 sm:p-6">
          <div className="animate-trace grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="rounded-doc border border-white/15 bg-white/10 p-6">
              <div className="flex items-start justify-between gap-4 border-b border-white/15 pb-4">
                <div>
                  ...
                  <h3 className="mt-1 font-display text-lg font-bold text-background">
                    Aménagement du cœur de village, Arzens (11)
                  </h3>
                </div>
                <span className="flex shrink-0 items-center gap-1.5 rounded-doc bg-success/15 px-2.5 py-1 text-[12px] font-semibold text-success">
                ...
                {extraits.map((item) => (
                  <li key={item.titre} className="flex flex-col gap-1 py-3.5">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="shrink-0 rounded-doc border border-white/15 bg-white/10 px-2 py-0.5 font-mono text-[11px] text-background/75">
                        {item.source}
                      </span>
                    </div>
                    <p className="text-[14px] leading-snug text-background/80">
                      {item.detail}
                    </p>
                  </li>
                ))}
            ...
            <div className="flex flex-col justify-center rounded-doc border border-dashed border-white/25 p-6">
              <div className="flex aspect-[4/3] max-w-sm flex-col items-center justify-center gap-3 self-center rounded-doc border border-white/15 bg-white/10 px-6 text-center">
                <span className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-background/40 text-background/75">
                  ▶
                </span>
                <p className="text-[13px] font-medium text-background/85">
                  Emplacement vidéo (à intégrer)
                </p>
                <p className="text-[12px] leading-snug text-background/70">
